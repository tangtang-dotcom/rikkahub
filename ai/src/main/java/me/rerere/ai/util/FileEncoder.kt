...
internal fun mapExifOrientationToTransform(orientation: Int): ExifTransformType =
    when (orientation) {
...
private fun sampleOptsFromFile(
    path: String,
    maxDimension: Int,
): BitmapFactory.Options {
    val probe = BitmapFactory.Options().apply { inJustDecodeBounds = true }
...
private fun applyExifTransform(
    ...
    when (transform) {
        ...
        ExifTransformType.FLIP_HORIZONTAL -> {
            matrix.setScale(-1f, 1f)
        }

        ExifTransformType.ROTATE_180 -> {
            matrix.setRotate(180f)
        }

        ExifTransformType.FLIP_VERTICAL -> {
            matrix.setScale(1f, -1f)
        }
...
private fun File.guessMimeType(): Result<String> =
    runCatching {
        inputStream().use { input ->
            ...
            if (bytes.copyOfRange(0, 4).toString(Charsets.US_ASCII) == "RIFF" && bytes
                    .copyOfRange(8, 12)
