...
fun compatExecute(id: String, index: Int, action: String, value: String?): AccessibilityActionResult {
    ...
    return when (action) {
        "tap" -> { val b=o.nodes.getOrNull(index)?.bounds ?: error("ACCESSIBILITY_NODE_NOT_FOUND"); AccessibilityActionEffects.showAction(s, action, b.centerX(), b.centerY(), b.centerX(), b.centerY(), 100L); s.clickNode(o,index).out(action) }
        "long_press" -> { val b=o.nodes.getOrNull(index)?.bounds ?: error("ACCESSIBILITY_NODE_NOT_FOUND"); AccessibilityActionEffects.showAction(s, action, b.centerX(), b.centerY(), b.centerX(), b.centerY(), 800L); s.longClickNode(o,index,800L).out(action) }
