...
fun createAndroidSystemTools(context: Context): List<Tool> = listOf(Tool(
    ...
    parameters = { InputSchema.Obj(properties = buildJsonObject {
        put("action", buildJsonObject { put("type", "string"); put("enum", buildJsonArray { add("volume"); add("set_volume"); add("media") }) })
        put("stream", buildJsonObject { put("type", "string"); put("enum", buildJsonArray { add("media"); add("alarm"); add("ring"); add("notification") }) })
