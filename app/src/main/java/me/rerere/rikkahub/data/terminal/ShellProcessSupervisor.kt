...
enum class TerminalEnvironment(val wireName: String) {
    ANDROID("android"),
    LINUX("linux"),
}
...
class ShellProcessSupervisor(
    ...
    private companion object {
        ...
        const val PROCESS_OWNER_ENV = "RIKKAHUB_PROCESS_OWNER"
    ...
    fun startShellProcess(
        ...
        val ownershipFile = runCatching {
            File.createTempFile("rikkahub-terminal-", ".owner")
    ...
    internal fun buildAndroidPayload(identity: String, command: String?): String {
        ...
        return "$discovery; " +
            "if [ -n \"${'$'}rikka_busybox\" ]; then " +
            "export RIKKAHUB_BUSYBOX=\"${'$'}rikka_busybox\" ASH_STANDALONE=1; " +
            "\"${'$'}rikka_busybox\" ash $shellArgument; " +
    ...
    internal fun buildLinuxPayload(rootfsPath: String, command: String?): String {
        ...
        val innerScript = """
            ...
            eta_mount_required() {
              ...
              eta_target=${'$'}2
              eta_options=${'$'}3
              "${'$'}rikka_busybox" mkdir -p "${'$'}eta_target" || exit 125
              "${'$'}rikka_busybox" mount -o "${'$'}eta_options" "${'$'}eta_source" "${'$'}eta_target" || exit 125
            }
            eta_mount_optional() {
              eta_source=${'$'}1
              eta_target=${'$'}2
              eta_options=${'$'}3
              "${'$'}rikka_busybox" mkdir -p "${'$'}eta_target" 2>/dev/null || return 0
              "${'$'}rikka_busybox" mount -o "${'$'}eta_options" "${'$'}eta_source" "${'$'}eta_target" 2>/dev/null || true
            }
            "${'$'}rikka_busybox" mount -t proc proc "${'$'}eta_rootfs/proc" || exit 125
        ...
        val discovery = AndroidBusyBox.discoveryScript()
        return "$discovery; " +
            "[ -n \"${'$'}rikka_busybox\" ] || { echo 'RIKKAHUB_LINUX_BUSYBOX_MISSING' >&2; exit 127; }; " +
            "eta_rootfs=$rootfs; " +
            "[ -f \"${'$'}eta_rootfs/${LinuxEnvironmentPaths.READY_MARKER}\" ] && " +
            "[ -x \"${'$'}eta_rootfs/bin/busybox\" ] || " +
            "{ echo 'RIKKAHUB_LINUX_ENVIRONMENT_NOT_READY' >&2; exit 127; }; " +
            "\"${'$'}rikka_busybox\" unshare -m --propagation private " +
            "\"${'$'}rikka_busybox\" sh -c ${shellQuote(innerScript)} eta-linux " +
            "\"${'$'}eta_rootfs\" \"${'$'}rikka_busybox\" $mode $payload"
    }
