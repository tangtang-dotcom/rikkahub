...
fun createAndroidStructuredCoreTools(context:Context,root:AndroidRootTerminalController):List<Tool>{
    ...
    return listOf(
        ...
        structuredTool("get_setting","Read an exact Android Settings value.",buildJsonObject{put("namespace",buildJsonObject{put("type","string");put("enum",buildJsonArray{add("system");add("secure");add("global")})});put("key",buildJsonObject{put("type","string")})},listOf("namespace","key")){o->
            val key=o.str("key")?.takeIf{it.matches(Regex("[A-Za-z0-9_.-]{1,200}"))}?:error("INVALID_ARGUMENT");val namespace=o.str("namespace")?:error("INVALID_ARGUMENT");val frameworkValue=when(namespace){"system"->Settings.System.getString(context.contentResolver,key);"secure"->Settings.Secure.getString(context.contentResolver,key);"global"->Settings.Global.getString(context.contentResolver,key);else->error("INVALID_ARGUMENT")};val value=frameworkValue?:root.executeSync(getSettingCommand(namespace,key),timeoutMs=10_000).takeIf{it.exitCode==0&&!it.timedOut}?.stdout?.trim()?.takeUnless{it.isBlank()||it=="null"};buildJsonObject{put("ok",true);put("tool","get_setting");put("namespace",namespace);put("key",key);value?.let{put("value",it)}?:put("value",JsonNull)}
        },
