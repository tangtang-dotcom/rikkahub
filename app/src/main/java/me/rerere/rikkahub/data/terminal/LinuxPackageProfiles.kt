...
internal class LinuxPackageProfileInstaller(
    ...
    fun isReady(): Boolean =
        LinuxEnvironmentPaths.packageProfileReady(
            LinuxEnvironmentPaths.rootfsDir(context).absolutePath,
            profile,
        )
    ...
    private suspend fun verifyAndMark(rootfs: File): Boolean {
        ...
        val command = """
            ...
            $checks
            cat > /${profile.markerName} <<'RIKKAHUB_PROFILE_EOF'
            profile=${profile.revision}
            RIKKAHUB_PROFILE_EOF
