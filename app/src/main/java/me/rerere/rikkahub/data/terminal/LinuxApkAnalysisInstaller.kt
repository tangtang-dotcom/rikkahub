...
internal class LinuxApkAnalysisInstaller(
    ...
    fun isReady(): Boolean =
        LinuxEnvironmentPaths.apkAnalysisReady(LinuxEnvironmentPaths.rootfsDir(context).absolutePath)
    ...
    private suspend fun installLocked(
        ...
        if (!LinuxEnvironmentPaths.commonToolsReady(rootfs.absolutePath)) {
    ...
    private fun prepareStaging(
        ...
        TerminalLogger.warn(
            "APK analysis profile action=prepare outcome=failed errorType=${throwable.javaClass.simpleName}",
        )
        staging.deleteRecursively()
        false
    }
    ...
    private suspend fun activateStaging(rootfs: File, staging: File): Boolean {
        val profileRoot = File(rootfs, "opt/rikkahub/apk-analysis")
        ...
        val command = """
            ...
            "${'$'}rikka_busybox" mkdir -p ${shellQuote(profileRoot.absolutePath)} || exit 70
            "${'$'}rikka_busybox" rm -rf ${shellQuote(installing.absolutePath)} ${shellQuote(previous.absolutePath)}
            "${'$'}rikka_busybox" mv ${shellQuote(staging.absolutePath)} ${shellQuote(installing.absolutePath)} || exit 71
            "${'$'}rikka_busybox" chmod 0755 \
            ...
            eta_link_commands() {
              "${'$'}rikka_busybox" mkdir -p ${shellQuote(File(rootfs, "usr/local/bin").absolutePath)} || return 1
              for eta_command in java jadx apktool smali baksmali; do
                "${'$'}rikka_busybox" rm -f ${shellQuote(File(rootfs, "usr/local/bin").absolutePath)}/"${'$'}eta_command"
              done
              "${'$'}rikka_busybox" ln -s ../../../opt/rikkahub/apk-analysis/current/bin/java \
                ${shellQuote(File(rootfs, "usr/local/bin/java").absolutePath)} || return 1
              "${'$'}rikka_busybox" ln -s ../../../opt/rikkahub/apk-analysis/current/jadx/bin/jadx \
                ${shellQuote(File(rootfs, "usr/local/bin/jadx").absolutePath)} || return 1
              for eta_command in apktool smali baksmali; do
                "${'$'}rikka_busybox" ln -s ../../../opt/rikkahub/apk-analysis/current/bin/"${'$'}eta_command" \
                  ${shellQuote(File(rootfs, "usr/local/bin").absolutePath)}/"${'$'}eta_command" || return 1
              done
            }
            eta_restore_previous() {
              "${'$'}rikka_busybox" rm -rf ${shellQuote(current.absolutePath)}
              if [ -d ${shellQuote(previous.absolutePath)} ]; then
                "${'$'}rikka_busybox" mv ${shellQuote(previous.absolutePath)} ${shellQuote(current.absolutePath)}
                eta_link_commands || true
              fi
            }
            if [ -d ${shellQuote(current.absolutePath)} ]; then
              "${'$'}rikka_busybox" mv ${shellQuote(current.absolutePath)} ${shellQuote(previous.absolutePath)} || exit 73
            fi
            "${'$'}rikka_busybox" mv ${shellQuote(installing.absolutePath)} ${shellQuote(current.absolutePath)} || {
              eta_restore_previous
              exit 74
            }
            eta_link_commands || {
            ...
            "${'$'}rikka_busybox" rm -f ${shellQuote(File(rootfs, LinuxEnvironmentPaths.APK_ANALYSIS_MARKER).absolutePath)}
        """.trimIndent()
        val result = InstallerShellRunner.run(
            command = command,
            timeoutSeconds = 60,
    ...
    private suspend fun verifyAndMark(rootfs: File): Boolean {
        val command = """
            ...
            cat > /${LinuxEnvironmentPaths.APK_ANALYSIS_MARKER} <<'RIKKAHUB_APK_ANALYSIS_EOF'
            profile=${LinuxEnvironmentPaths.APK_ANALYSIS_REVISION}
            jadx=$JADX_VERSION
            apktool=$APKTOOL_VERSION
            smali=$SMALI_VERSION
            RIKKAHUB_APK_ANALYSIS_EOF
    ...
    private suspend fun cleanupAfterSuccess(rootfs: File, artifacts: Collection<File>) {
        ...
        val previous = File(rootfs, "opt/rikkahub/apk-analysis/previous")
        val command = """
            ${AndroidBusyBox.discoveryScript()}
            [ -n "${'$'}rikka_busybox" ] || exit 127
            "${'$'}rikka_busybox" rm -rf ${shellQuote(previous.absolutePath)}
        """.trimIndent()
        InstallerShellRunner.run(command, 30, TerminalEnvironment.ANDROID)
    }
    ...
    companion object {
        ...
        internal val APKTOOL_WRAPPER = """
            ...
            case "${'$'}{1:-}" in
              b|build)
                echo "APKTOOL_BUILD_UNAVAILABLE: RikkaHub APK 分析环境暂不包含 ARM64 AAPT2，仅支持解码与检查。" >&2
                exit 64
                ;;
            esac
            exec java -jar /opt/rikkahub/apk-analysis/current/lib/apktool.jar "${'$'}@"
        """.trimIndent() + "\n"
        ...
        private fun javaJarWrapper(fileName: String): String =
            "#!/bin/sh\nexec java -jar /opt/rikkahub/apk-analysis/current/lib/$fileName \"${'$'}@\"\n"
    }
}
