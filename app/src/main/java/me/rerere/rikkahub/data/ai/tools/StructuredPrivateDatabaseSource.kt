...
internal class StructuredPrivateDatabaseSource(
    ...
    private fun createSnapshot(source: String, maxBytes: Long): File? {
        ...
        val result = root.executeSync(command, timeoutMs = 15_000L)
        if (result.exitCode == 0 && !result.timedOut) return snapshot
        deleteSnapshot(snapshot)
        return null
    }
