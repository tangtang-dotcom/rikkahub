...
internal object AndroidBusyBox {
    ...
    fun discoveryScript(variable: String = "rikka_busybox"): String {
        require(variable.matches(Regex("[a-z_][a-z0-9_]*"))) { "非法 Shell 变量名" }
        val quotedCandidates = candidates.joinToString(" ") { shellQuote(it) }
        return "$variable=''; " +
            "for rikka_candidate in $quotedCandidates; do " +
            "if [ -x \"${'$'}rikka_candidate\" ]; then $variable=\"${'$'}rikka_candidate\"; break; fi; " +
            "done; " +
