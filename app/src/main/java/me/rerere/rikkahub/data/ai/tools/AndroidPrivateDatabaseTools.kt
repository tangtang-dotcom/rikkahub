...
fun createAndroidPrivateDatabaseTools(context:Context,root:AndroidRootTerminalController):List<Tool>{
    ...
    return listOf("list_alarms","list_active_timers","search_clipboard_history","get_health_summary").map{name->Tool(
        name=name,description="Read a bounded snapshot of a verified local Android/ColorOS database using Eta's fixed schema contract.",
