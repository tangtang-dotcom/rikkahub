        ...
        if (tg.enabled) {
            add(
                DoctorCheck(
                    ...
                    detail = if (tg.token.isNotBlank()) "已配置令牌（${tg.token.length} 字符，已隐藏）。"
                    else "Telegram 机器人已启用但未设置令牌 — 服务将在启动时失败。",
            ...
            add(
                DoctorCheck(
                    ...
                    else "服务已停止。Telegram 消息将无法送达助手。看门狗将在下次 30 分钟健康检查时重试。",
                    severity = when {
    ...
    private suspend fun networkChecks(): List<DoctorCheck> = buildList {
        runCatching {
            ...
            add(
                DoctorCheck(
                    ...
                    fix = FixAction.OpenAppRoute("打开供应商", AppRouteKey.SettingProvider),
                )
            )
        }
        ...
        add(
            DoctorCheck(
                ...
                else "DNS 解析失败或超时。网络变化时 NetworkChangeMonitor 会清空 OkHttp 连接池 — 如果持续红色，请检查网络连接。",
    ...
    private fun termuxChecks(enabled: Set<LocalToolOption>): List<DoctorCheck> = buildList {
        ...
        add(
            DoctorCheck(
                ...
                else "Termux 未安装。需求方: ${needers.joinToString(", ") { it.shortName() }}.",
    ...
    private fun browserChecks(enabled: Set<LocalToolOption>): List<DoctorCheck> = buildList {
        ...
        if (prefs != null) {
            ...
            val detail = if (onWriteTools.isEmpty())
                ...
                "已启用的副作用浏览器工具数: ${onWriteTools.size} (${onWriteTools.joinToString(", ") { it.removePrefix("browser_") }})."
            add(
                DoctorCheck(
