...
- **功能** OpenAI 兼容格式接入支持 — provider 新增「工具名前缀」配置（toolNamePrefix，默认空）：填 `rh_` 时发送加前缀 / 解析剥前缀 / 回传加回，解决客户端工具名与 OpenClaw 内置/保留工具名冲突（`invalid tool configuration` 400），恢复「工具选项」可用，OpenClaw 类兼容端点接入后工具可正常调用（`91a4e9fe`）
- **修复** 工具名前缀参数贯穿 buildMessages/addAssistantMessages 签名 + 反射测试同步（`3237e095`）
...
- **修复** SSH 主机列表不显示 — CardGroup 动态收集（forEach+DSL item）在 LazyColumn 惰性组合下不渲染（功能上线以来一直存在，getAll 正常但 UI 空），改 LazyColumn items() 直接渲染 + ListItem（`8a602213`）
