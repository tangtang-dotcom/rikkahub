...
class LiteRtProvider(
        ...
        flow {
            ...
            val systemTextsRaw =
                messages
                    ...
                    .joinToString("\n\n")
            val trimmedSystemTexts =
                if (systemTextsRaw.length > SYSTEM_MESSAGE_CHAR_BUDGET) {
            ...
            val visionPersistentlyUnavailable =
                runCatching {
            ...
            if (outcome.visionFellBackToTextOnly && !visionPersistentlyUnavailable) {
                Log.w(
                    TAG,
                    "Vision encoder unavailable on this device for ${params.model.modelId}; " +
            ...
            val turnImages: List<android.graphics.Bitmap> =
                if (imageDecision.forwardImages) {
                ...
                try {
                    runtime
                        .streamTurns(
                            ...
                            val delta =
                                if (cumulative.startsWith(previousCumulative)) {
            ...
            runtime.lastTelemetry?.let { tele ->
                ...
                if (tele.decodeMs > 100 && tele.outputCharCount > 0) {
                    ...
                    Log.i(
                        TAG,
                        "telemetry: prefill=${"%.2f".format(
                            tele.prefillTps,
                        )} tok/s decode=${"%.2f".format(tele.decodeTps)} tok/s " +
     ...
     */
    private fun renderTurnRawText(msg: UIMessage): String =
        buildString {
            when (msg.role) {
                MessageRole.USER -> {
                    append(
    ...
    private fun renderColdBlob(messages: List<UIMessage>): String =
        if (messages.size == 1) {
            renderTurnRawText(messages[0])
        } else {
            renderHistoryAsChatML(messages)
        }

    private fun renderHistoryAsChatML(messages: List<UIMessage>): String =
        buildString {
            var first = true
            for (msg in messages) {
                ...
                when (msg.role) {
                    MessageRole.USER -> {
                        append("User: ").append(renderTurnRawText(msg))
                    }

                    MessageRole.ASSISTANT -> {
                        append("Assistant: ").append(renderTurnRawText(msg))
                    }
     ...
     */
    private fun translateSdkError(
        t: Throwable,
        effectiveMaxNumTokens: Int,
    ): Throwable {
        ...
        if (joined.contains("Input token ids are too long", ignoreCase = true) ||
            ...
            val (have, limit) =
                overflow?.let { it.groupValues[1] to it.groupValues[2] }
