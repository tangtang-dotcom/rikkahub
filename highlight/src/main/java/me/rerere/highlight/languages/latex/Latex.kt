...
internal fun latex(): Language {
    ...
    fun verbatimDelimitedEqual(innerName: String = "string"): Mode =
        mode {
            scope = innerName
            begin = """(.|\r?\n)"""
            end = """(.|\r?\n)"""
    ...
    fun verbatimDelimitedEnvironment(name: String): Mode =
        mode {
            scope = "string"
            end = """(?=\\end\{$name\})"""
        }

    fun verbatimDelimitedBraces(innerName: String = "string"): Mode =
        mode {
            relevance = 0.0
            begin = """\{"""
            starts =
