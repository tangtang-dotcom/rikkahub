...
internal fun php(): Language {
    ...
    val leftAndRightSideOfDoubleColon =
        mode {
            variants =
                listOf(
                    {
...
private fun dualCase(items: List<String>): List<String> =
    buildList {
        items.forEach { item ->
            add(item)
...
private fun normalizeKeywords(items: List<String>): List<String> = items.map { it.replace(Regex("""\|\d+$"""), "") }
