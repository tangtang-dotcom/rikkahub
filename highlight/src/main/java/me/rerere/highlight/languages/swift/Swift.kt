...
internal fun swift(): Language {
    ...
    fun escapedCharacter(rawDelimiter: String = "") =
        mode {
            scope = "subst"
            variants =
                listOf(
    ...
    fun escapedNewline(rawDelimiter: String = "") =
        mode {
            scope = "subst"
            match = concat("""\\""", rawDelimiter, """[\t ]*(?:[\r\n]|\r\n)""")
        }

    fun interpolation(rawDelimiter: String = "") =
        mode {
            scope = "subst"
            label = "interpol"
    ...
    fun multilineString(rawDelimiter: String = "") =
        mode {
            begin = concat(rawDelimiter, "\"\"\"")
    ...
    fun singleLineString(rawDelimiter: String = "") =
        mode {
            begin = concat(rawDelimiter, "\"")
            end = concat("\"", rawDelimiter)
            contains =
    ...
    val implicitParameter =
        mode {
            scope = "variable"
            match = """\${'$'}\d+"""
        }
    val propertyWrapperProjection =
        mode {
            scope = "variable"
    ...
    val typeDeclaration =
        mode {
            ...
            keywords = swiftKeywords
            contains = listOf(genericParameters) + keywordModes +
                mode {
                    begin = ":"
                    end = """\{"""
                    keywords = swiftKeywords
...
private val PRECEDENCE_GROUP_KEYWORDS =
    listOf(
        "assignment",
        "associativity",
        "higherThan",
...
private val NUMBER_SIGN_KEYWORDS =
    listOf(
        "#colorLiteral",
        "#column",
        "#dsohandle",
...
private val OPERATOR_CHARACTER =
    either(
        OPERATOR_HEAD,
        """[\u0300-\u036F]""",
        """[\u1DC0-\u1DFF]""",
...
private val OPERATOR_IDENTIFIER_CHARACTER =
    either(
        IDENTIFIER_HEAD,
        """\d""",
        """[\u0300-\u036F\u1DC0-\u1DFF\u20D0-\u20FF\uFE20-\uFE2F]""",
...
private val KEYWORD_ATTRIBUTES =
    listOf(
        "attached",
        "autoclosure",
...
private val AVAILABILITY_KEYWORDS =
    listOf(
        "iOS",
